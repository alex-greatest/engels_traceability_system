...
import { useComponentsBinding } from 'Frontend/components/api/component_binding';
import { useComponents } from 'Frontend/components/api/components_type';
...
const ComponentsBinding = () => {
  ...
  const nameStation = useSignal<string>("Рабочее место 1");
  ...
  const { data: componentsBinding, isError, isLoading, refetch, isRefetching } = useComponentsBinding(nameStation.value);
  const { data: componentsType, isError: isErrorType, isLoading: isLoadingTyp, refetch: refetchType, isRefetching: isRefetchingType } = useComponents();
  ...
  const defaultProps = {
    options: componentsType ?? {} as ComponentTypeDto[],
