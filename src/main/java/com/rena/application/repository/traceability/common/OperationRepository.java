...
public interface OperationRepository extends JpaRepository<Operation, Long> {
    ...
    @Query("select o from Operation o " +
            "JOIN o.station JOIN o.boiler" +
            " where o.status = ?1 and o.station.name = ?2 and o.boiler.serialNumber = ?3 and o.isLast = true")
    Optional<Operation> findByStatus_IdAndStation_NameAndIsLastTrue(Integer status, String name, String serialNumber);

    @Query("select o from Operation o JOIN o.station JOIN o.boiler.boilerTypeCycle where o.id = ?1 and o.isLast = true")
